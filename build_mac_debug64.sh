...
mkdir -p $PROJECT_DIR/build/odbc/logs

mkdir $BUILD_DIR
cd $BUILD_DIR
cmake ../src -DCMAKE_PREFIX_PATH="${PROJECT_DIR}/${VCPKG_INSTALLED_DIR}" -DCMAKE_BUILD_TYPE="$BUILD_TYPE" -DCODE_COVERAGE="ON" -DBUILD_SHARED_LIBS="OFF" -DWITH_TESTS="ON" -DWITH_ODBC="ON"
make -j 4
