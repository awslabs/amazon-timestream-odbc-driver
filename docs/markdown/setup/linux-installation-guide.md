...
2. Install the package
    ...
    ```
    sudo dpkg -i AmazonTimestreamODBC_2.1.4_RELEASE_amd64.deb
    ```

    #### Sample command for Debian 32 bit 
    ```
    sudo dpkg -i AmazonTimestreamODBC_2.1.4_RELEASE_i386.deb
    ```

    #### Sample command for RPM based 64 bit
    ```
    sudo rpm -i AmazonTimestreamODBC_2.1.4_RELEASE_x86_64.rpm
    ```

    #### Sample command for RPM based 32 bit
    ```
    sudo rpm -i AmazonTimestreamODBC_2.1.4_RELEASE_i386.rpm
    ```
...
1. Remove the package from repository
    ...
    ```
    sudo zypper remove amazontimestreamodbc_2.1.4_release_x86_64
    ```
    or

    ```
    sudo yum remove amazontimestreamodbc_2.1.4_release_x86_64
    ```

    #### Command for RPM based 32 bit
    ```
    sudo zypper remove amazontimestreamodbc_2.1.4_release_i386
    ```
    or

    ```
    sudo yum remove amazontimestreamodbc_2.1.4_release_i386
    ```

    #### Command for Debian 64 bit
    ```
    sudo apt remove amazontimestreamodbc_2.1.4_release_amd64
    ```

    #### Command for Debian 32 bit
    ```
    sudo apt remove amazontimestreamodbc_2.1.4_release_i386
    ```
...
4. Remove the libraries installed to /usr/lib if they exist
    ```
    sudo rm /usr/lib/libtimestream-odbc.so /usr/lib/libtimestream-odbc.so.2.1.4
    ```
